...
__global__ void __launch_bounds__(kTRI)
    resampleSystematicIndexUp(int const num_particles, unsigned long long int const seed,
                              int* __restrict__ resampling_index_up, float* __restrict__ prefix_sum)
{
    ...
    for (unsigned int tid = blockIdx.x * blockDim.x + threadIdx.x; tid < num_particles; tid += blockDim.x * gridDim.x)
    {
...
__device__ void ResamplingUpPerWarp(cg::thread_block_tile<kWarpSize> const& tile_32, unsigned int const& tid,
                                    int const& num_particles, float const& distro, float* shared,
                                    float* __restrict__ prefix_sum, int* __restrict__ resampling_index_up)
{
    ...
    {  // strange diff kWarpSize or kTRI
...
__global__ void __launch_bounds__(kTRI)
    resampleSystematicIndexDown(int const num_particles, unsigned long long int const seed,
                                int* __restrict__ resampling_index_down, float* __restrict__ prefix_sum)
{
...
__device__ void ResamplingDownPerWarp(cg::thread_block_tile<kWarpSize> const& tile_32, unsigned int const& tid,
                                      int const& num_particles, float const& distro, float* shared,
                                      float* __restrict__ prefix_sum, int* __restrict__ resampling_index_down)
{
