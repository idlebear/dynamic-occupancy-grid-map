    ...
    for (int x = start_x; x < grid_size; x += stride_x)
    {
        for (int y = start_y; y < grid_size; y += stride_y)
        {
            ...
            if (translated_theta >= 0 && theta1 < polar_width && r1 < polar_height)
            {
                ...
                meas_grid.occ_mass[index] = m11.x * theta1_prop * r1_prop + m12.x * theta1_prop * r2_prop +
                                            m21.x * theta2_prop * r1_prop + m22.x * theta2_prop * r2_prop;
                meas_grid.free_mass[index] = m11.y * theta1_prop * r1_prop + m12.y * theta1_prop * r2_prop +
                                             m21.y * theta2_prop * r1_prop + m22.y * theta2_prop * r2_prop;
